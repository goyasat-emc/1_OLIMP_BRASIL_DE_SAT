...
const int entrada_sensores_ds18b20 = 13;

// Correlação linear simples
// y = ax + b, onde y corresponde ao sensor embutido e x corresponde aos sensores ds18b20
// Além disso, x = (y - b)/a
float a = 0.2867;
float b = 6.4901;
...
float ds18b20_sensor0, ds18b20_sensor1, ds18b20_sensor2, ds18b20_sensor3, ds18b20_sensor4;

String json;

OneWire oneWire(entrada_sensores_ds18b20);  /*Protocolo OneWire*/
...
void setup() {
  ...
  pinMode(entrada_sensores_ds18b20, INPUT);
...
void loop() {
  ...
  ds18b20_sensor0 = sensorTemp.getTempCByIndex(0);
  ds18b20_sensor1 = sensorTemp.getTempCByIndex(1);
  ds18b20_sensor2 = sensorTemp.getTempCByIndex(2);
  ds18b20_sensor3 = sensorTemp.getTempCByIndex(3);
  ds18b20_sensor4 = sensorTemp.getTempCByIndex(4);
...
void getSensorsInfo() {
  ...
  // x = (y - b)/a
  temperatura = (cubeSat.getTemperature() - b)/a;
...
void createFileFirstLine(fs::FS &fs, const char * path) {
  ...
  const char * message = "horario(ms),equipe,bateria(%),temperatura(C),pressao(Pa),velAngularX(graus/s),velAngularY(graus/s),velAngularZ(graus/s),aceleracaoX(m/s2),aceleracaoY(m/s2),aceleracaoZ(m/s2),numImagens, ds18b20_sensor0, ds18b20_sensor1, ds18b20_sensor2, ds18b20_sensor3, ds18b20_sensor4";
...
void appendFile(fs::FS &fs, const char * path, TickType_t time) {
  ...
  file.write(',');
  file.print(ds18b20_sensor0);
  file.write(',');
  file.print(ds18b20_sensor1);
  file.write(',');
  file.print(ds18b20_sensor2);
  file.write(',');
  file.print(ds18b20_sensor3);
  file.write(',');
  file.println(ds18b20_sensor4);
  // Fecha o arquivo
  file.close();
}
...
void sendDataWs() {
  ...
  jsonBuffer["equipe"] = equipe;
  jsonBuffer["numImagens"] = numImagens;
  jsonBuffer["bateria"] = bateria;
  jsonBuffer["pressao"] = pressaoAtmosferica;
  jsonBuffer["temperatura"] = temperatura;
  jsonBuffer["humidade"] = umidade;
  jsonBuffer["co2"] = nivelCO2;
  jsonBuffer["luminosidade"] = luminosidade;
  // Adiciona as leituras a um array dentro do JSON
  JsonArray ds18b20 = jsonBuffer.createNestedArray("ds18b20");
    ds18b20.add(ds18b20_sensor0);
    ds18b20.add(ds18b20_sensor1);
    ds18b20.add(ds18b20_sensor2);
    ds18b20.add(ds18b20_sensor3);
    ds18b20.add(ds18b20_sensor4);
