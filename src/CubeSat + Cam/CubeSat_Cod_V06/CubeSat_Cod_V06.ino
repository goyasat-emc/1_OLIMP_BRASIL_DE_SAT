...
extern uint8_t pictureNumber;

// Correlação linear simples (desconsidera nesta versão do código)
// y = ax + b, onde y corresponde ao sensor embutido e x corresponde aos sensores ds18b20
// Além disso, x = (y - b)/a
// const float a = 0.2867;
// const float b = 6.4901;

// Informações obrigatórias de status do satélite
const uint8_t equipe = 91; // Cada equipe receberá um ID, que deverá ser informado, obrigatoriamente, no campo "equipe"
...
// 4 minutos = 240000ms
unsigned long timerDelay = 10; //240000;

unsigned long timerDelay_SD = 50000; // Intervalo de gravação de dados no cartão micro SD
...
void loop() {
  ...
  for(int i = 0; i<5; i++){
    ds18b20[i] = sensorTemp.getTempCByIndex(i);
  }
...
String getCubeSatJson() {
  ...
  json += "],\"payload\":{\"pictureNumberImagens\":";
  json += String(pictureNumber);
...
void appendFile(fs::FS &fs, const char * path, TickType_t time) {
  ...
  for(int i = 0; i<5; i++){ // Registra no arquivo as leituras dos sensores ds18b20
    file.print(ds18b20[i]);
    file.write(',');
  }
