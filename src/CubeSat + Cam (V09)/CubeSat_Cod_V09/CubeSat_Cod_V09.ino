...
void setup() {
  ...
  // Configura o terminal do sensor de temperatura DS18B20 como entrada
  ...
  // Inicializa o protocolo de comunicação sem fio ESP-NOW
...
void loop() {
  sensorTemp.requestTemperatures(); /* Envia o comando para leitura da temperatura e armazena na variável ds18b20 */
  ...
  if ((millis() - lastTime) > timerDelay) {
    ...
    lastTime = millis(); // O método millis() retorna o tempo em ms desde que o código começou a ser executado
  }
}

// Função que armazena em variáveis as leituras de todos os sensores colocados abaixo
void getSensorsInfo() {
...
String getCubeSatJson() {
  // Atualização dos dados dos sensores em suas respectivas variáveis
  getSensorsInfo();

  // Criação da string no formato JSON a ser enviada via HTTP POST
  json = "{\"equipe\":";
  json += String(equipe);
  json += ",\"bateria\":";
  json += String(bateria);
...
// Modificação da função de cria a primeira linha dos arquivos '.csv' no cartão de memória
void createFileFirstLine(fs::FS &fs, const char * path) {
  ...
  const char * message = "horario(ms),equipe,bateria(%),temperatura(C),ds18b20,pressao(Pa),velAngularX(graus/s),velAngularY(graus/s),velAngularZ(graus/s),aceleracaoX(m/s2),aceleracaoY(m/s2),aceleracaoZ(m/s2),numImagens,instante_ultimaFoto(ms),JSON";
